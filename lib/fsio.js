 ...
 */
function sandboxPolicyOf(ctx, exec) {
    let svc = ctx?.sandboxPolicy;
    ...
    try {
        return svc.resolve(session === undefined ? {} : { session });
    } catch {
        return undefined;
    }
}
 ...
 * 裸后端对多余的第 5 参视而不见，无回归。FS_SANDBOX_DENIED 在此改写为
 * 可行动提示（保留宿主原文），其余错误原样上抛。
 */
async function writeGuarded(ctx, exec, target, text, intent, sig) {
    const policy = sandboxPolicyOf(ctx, exec);
    try {
        return policy === undefined
            ? await ctx.fs.writeText(target, text, intent, sig)
            : await ctx.fs.writeText(target, text, intent, sig, policy);
    } catch (error) {
        if (error instanceof Error && error.message.includes('file access denied under')) {
            throw new Error(
                `${error.message}（dsh-novel-forge：这是文件沙箱策略拒绝，不是磁盘权限问题；`
                + '常见于 dsh web 的启动目录 ≠ 会话工作区（Windows 典型）。解法任选：'
                + '①把该会话的文件策略切到 danger-full-access；②从会话工作区目录启动 dsh web；'
                + '③更新本插件（写盘调用已线程会话策略）。）'
            );
        }
        throw error;
...
export function createServerFsio(ctx, cwd) {
    ...
    const api = {
        ...
        async writeText(p, text, mode = 'auto') {
            ...
            return writeGuarded(ctx, undefined, target, text, intent);
        },
        async writeJson(p, value, mode = 'auto') {
            return api.writeText(p, `${JSON.stringify(value, null, 2)}\n`, mode);
        },
