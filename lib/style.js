...
export function judgeAgainstBaseline(metrics, baseline, toleranceOverrides = {}) {
    ...
    for (const { key, label, unit } of STYLE_DIMENSIONS) {
        // 旧版本基线可能缺字段（sigma/tolerance/mu）——逐项兜底，杜绝 undefined
        // 混进工具输出（宿主 lossless JSON 拒收 undefined，报「not lossless JSON」）
        const raw = baseline.dims[key];
        const base = (raw && typeof raw === 'object')
            ? { mu: Number.isFinite(raw.mu) ? raw.mu : metrics[key], sigma: raw.sigma ?? null, tolerance: Number.isFinite(raw.tolerance) ? raw.tolerance : 35 }
            : { mu: metrics[key], sigma: null, tolerance: 35 };
