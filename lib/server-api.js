...
//       SessionStore 注册名 "sessions"、`list()` 可调（dsh-session L1311-1315）。浏览器半的
//       `ctx.sessions`（dsh-api-session-controller/client.js → 网关 RemoteSnapshot）是**另一套**，
//       别混为一谈；宿主 REST 看到的是前者。
//    b. 但 store 是**内存态、由创建 fiber 持有**（SessionStore 头注释原文：owned by the calling
//       fiber, disposing removes from the store）→ 会话只在**agent 轮运行期间**在册，面板空闲期
//       REST 读到的 store≈空。⇒ live 的命中窗口 = 「拥有该工作区的会话正在跑」（典型：agent 刚把
//       书建进新工作区，运行中面板刷新立即可见）；**空闲期新会话工作区进场的真机路径是源①目录态
//       落盘**——所以持久层缓存必须带内容签名失效（见 persistentSig）。live 每次现算零成本。
    ...
    ctx.inject(['webServer'], (wctx) => {
        ...
        const decodeWorkspaceRoots = () => {
            ...
            try {
                ...
                else if (raw.length === 0) liveHint = '（无正在运行的 agent 轮——store 由 fiber 持有，空闲期≈空，属预期）';
