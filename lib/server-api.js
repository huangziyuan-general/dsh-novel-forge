...
export function registerServerApi(ctx, config, deps = {}) {
    ...
    ctx.inject(['webServer'], (wctx) => {
        ...
        const decodeWorkspaceRoots = () => {
            ...
            _rootsTs = now;
            // 扫描根变化时打一行（含首次）：真机排查「面板无书」时，这行直接回答
            // 「三源合并到底找到了哪些工作区」——Windows 上 live sessions 源是否命中
            // 一眼可见
            const rootsKey = _rootsCache.join('\n');
            if (rootsKey !== _lastRootsKey) {
                _lastRootsKey = rootsKey;
                console.info(`[novel-forge] 扫描根更新（${_rootsCache.length} 个）：${_rootsCache.join(' | ') || '（无）'}`);
            }
            return _rootsCache;
        };
