...
import { createServerFsio, auditLine, assertBookName } from './fsio.js';
...
export function registerServerApi(ctx, config, deps = {}) {
    ...
    ctx.inject(['webServer'], (wctx) => {
        ...
        const decodeWorkspaceRoots = () => {
            ...
            try {
                const decoded = readdirSync(SESSIONS_DIR)
                    .filter((d) => d.startsWith('--') && d.endsWith('--') && d.length > 4)
                    .map((d) => `/${d.slice(2, -2).replace(/-/g, '/')}`);
                // L3：解码是有损的（真实目录名含 - 时解不出来）——验证失败的跳过，但
                // **要留痕**：书「静默从面板消失」排查过一轮（审查 L3），warn 一声省得再猜
                _rootsCache = decoded.filter((p) => {
                    try { return statSync(p).isDirectory(); } catch {
                        console.warn(`[novel-forge] 会话工作区解码后不存在（目录名含「-」会解码歧义，已跳过）：${p}`);
                        return false;
                    }
                });
            } catch { _rootsCache = []; }
            ...
            handler: async (req, res) => {
                ...
                if (req.method === 'POST' && segments[0] === 'projects' && segments.length === 1) {
                    ...
                    try {
                        ...
                        const slug = assertBookName(title.replace(/[\/\\:*?"<>|]/g, '_'));
                ...
                if (req.method === 'POST' && segments[0] === 'projects' && segments[2] === 'chapters' && segments.length === 4) {
                    ...
                    try {
                        ...
                        const rel = `${bookId}/正文/${fileName}`;
                        await fsio.writeText(rel, content);
                        const rec = store.chapterRecord(prev, { title, version, file: rel, chars: content.replace(/\s/g, '').length });
                        ...
                        await fsio.appendLine(`${bookId}/.novel/audit.jsonl`,
                            auditLine('rest/chapter_save', { chapter: chapterNo, version, title, chars: rec.chars }, 'user'));
