...
export function registerServerApi(ctx, config, deps = {}) {
    ...
    ctx.inject(['webServer'], (wctx) => {
        ...
        // 持久源（projcache 双形态 + 目录名反推）= 盘上状态，60s TTL 缓存合理；
        // live 源**不进缓存**——审查（db4f1b1 复盘）揪出的真 bug：整包缓存曾把新开会话的
        // cwd 一起锁 60s，新会话建书后面板空 60s 才自愈，恰好延长本批要消灭的症状
        const decodePersistentRoots = () => {
            const now = Date.now();
            if (_persistCache !== null && now - _persistTs < ROOTS_TTL_MS) return _persistCache;
        ...
        const decodeWorkspaceRoots = () => {
            ...
            // 「三源各自命中多少、最终用了哪些工作区」
            const summary = `扫描根（live=${live.length}${liveHint} 持久源=${persist.length} → 有效 ${_rootsCache.length} 个）`
