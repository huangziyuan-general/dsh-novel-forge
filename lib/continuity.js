...
/** 死亡词。注意「战死」与反例里的「死战」是不同串，互不干扰。 */
const DEATH_RE = /(阵亡|战死|身亡|身死|殒命|殒身|命殒|毙命|陨落|已逝|去世|牺牲|殉难|殉职|殉道|遇害|气绝|驾崩|道消|玉殒|死了|已死|死亡|殁)/;
/** 反例：这些词里有「死」但不是死亡。 */
 ...
 * 死后的**账本**状态变更（B2）。
 *
 * 与 {@link scanDeadReappear} 的分工——两者互补，不是重复：
 *   - scanDeadReappear 扫**正文里的实体名**，会误伤「有人提起亡者」「灵位/遗物」这类合法叙事；
 *   - 这里扫**模型主动落的账**（entity|key|value），语义明确、误报率低，
 *     且不依赖正文是否写了名字——「第 40 章断腿第 50 章还跑」这类**非死亡**的状态矛盾，
 *     只有账本推演能抓到。
 *
 * 判据保守：死亡章之后该实体仍有**非死亡类**新记录即提示（warning 而非 error），
 * 因为「遗物被发现」「传闻已死」本身也是合法记录。
 */
export function posthumousChanges(facts) {
    const deaths = deathTimeline(facts);
    const issues = [];
    for (const [entity, d] of deaths) {
        const later = (facts ?? []).filter((f) => f !== null && typeof f === 'object'
            && f.entity === entity && Number.isInteger(f.chapter)
            && f.chapter > d.chapter && !isDeathRecord(f));
        if (later.length === 0) continue;
        const first = later.reduce((m, f) => (f.chapter < m.chapter ? f : m));
        issues.push({
            severity: 'warning',
            code: 'posthumous-change',
            where: `第${first.chapter}章·${entity}`,
            message: `「${entity}」已在第${d.chapter}章${d.value}（${d.key}），第${first.chapter}章账本又记了新状态「${first.key}: ${first.value}」——`
                + '若确为遗物/传闻/闪回请忽略，否则是「死亡后仍在活动」的硬伤',
        });
    }
    return issues;
}
