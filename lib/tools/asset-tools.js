...
export function defineImportTool(ctx, config) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            // 章节索引（以切分后的连续章号为准）
            const novel = { ...defaultNovel({ title, genre: args.genre ?? '未分类', logline: args.logline ?? inferOpeningPreview(raw) ?? '' }), chapters: {} };
            for (const c of chapters) {
                const rel = p.chapterFile(c.n, c.title, 1);
...
export function defineCloneTool(ctx, config) {
    return defineTool({
        ...
        isConcurrencySafe: () => false,
        async execute(args, exec) {
            const fromBook = assertBookName(args.from_book);
            const newBook = assertBookName(args.new_book);
            if (fromBook === newBook) throw new Error('源与目标书目不能相同');
            const io = createFsio(ctx, exec, sessionCwd(exec));
            const sp = pathsFor(fromBook);
            const dp = pathsFor(newBook);
            if ((await io.readJson(dp.meta)) !== null) throw new Error(`目标书目已存在：「${newBook}」`);
            const src = await io.readJson(sp.meta);
            if (src === null) throw new Error(`源书目不存在：「${fromBook}」`);

            let missing = 0; let chapters = 0;

            // 章节文件（按版本）——重建干净的 index 记录，避免复用旧记录的脏字段
            const newChapters = {};
            for (const [k, rec] of Object.entries(src.chapters ?? {})) {
                const files = [];
                for (const f of rec.files ?? []) {
                    if (!f.file.startsWith(`${fromBook}/`)) { missing += 1; continue; }
                    const content = await io.readText(f.file);
                    if (content === null) { missing += 1; continue; }
                    const toPath = f.file.replace(`${fromBook}/`, `${newBook}/`);
            ...
            // 全包围/summary not just chapterRecord shape: rebuild cleanly
            // approvals 必须带 outline 子对象（与 defaultNovel 同形）——否则新书
            // novel_outline approve 写 novel.approvals.outline[n] 时直接 TypeError。
            const novel = {
                title: args.title ?? newBook,
                genre: src.genre ?? '未分类',
                logline: src.logline ?? '',
                stage: 'planning',
            ...
            // 世界书/账本/伏笔/术语表
            for (const [from, to] of [[sp.worldbook, dp.worldbook], [sp.facts, dp.facts], [sp.foreshadows, dp.foreshadows], [sp.glossary, dp.glossary]]) {
                const content = await io.readText(from);
                if (content !== null) await io.writeText(to, content, 'create');
            }
            await io.writeJson(dp.meta, novel, 'create');
            await audit(io, dp, 'project/clone', { from: fromBook, to: newBook, chapters });
            return { book: newBook, from_book: fromBook, action: 'clone', chapters, missing, new_stage: 'planning', next: `克隆完成：《${novel.title}》已就绪（阶段重置 planning，提案已清空）` };
        },
