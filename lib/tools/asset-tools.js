...
export function defineExportTool(ctx, config) {
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            // L12 修复：file 覆盖路径必须落在本书目录内——不然「导出」可以写工作区里
            // 任何可写文件（覆盖 novel.json / 伏笔.json / 别的书的正文）。
            if (rel !== `${book}/导出/《${sanitizeTitle(novel.title)}》.${format}`
                && !(rel.startsWith(`${book}/`) && !rel.slice(book.length + 1).includes('..'))) {
                throw new Error(`file 必须落在本书目录内（${book}/…），收到：${rel}`);
            }
