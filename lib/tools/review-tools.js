...
import { analyzeParagraphs } from '../polish.js';
...
export function definePolishTool(ctx, config) {
    return defineTool({
        name: 'novel_polish',
        description: '润色定位 + 提交：analyze 返回本章段落级病灶（AI 味/长段/灌输腔/章末钩子）；submit 把模型改好的整章文本作为「润色提案」提交（走提案制、用户确认才 apply 新版本，永不覆盖旧版）。',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            await audit(io, p, 'polish/submit', { id, chapter: args.chapter, chars });
            return {
                book, action: 'submit', chapter: args.chapter, id, status: 'pending', previous_version: rec.latest,
                next: `润色提案 ${id} 已登记——用户确认后 novel_propose apply 生成 v${rec.latest + 1}`,
            };
        },
        render: (_args, v) => textBlock(
            v.action === 'analyze'
                ...
                : `已提交润色提案 ${v.id}（v${v.previous_version} → 待 apply 成 v${v.previous_version + 1}），走提案制不改原稿`
        ),
    });
}