    ...
    return defineTool({
        ...
        async execute(args, exec) {
            ...
            const resolveIndexPath = async () => {
                ...
                if (typeof io.cwd === 'string' && io.cwd !== '') return join(io.cwd, p.indexDb);
                return null;
            };
            ...
            if (args.action === 'build') {
                ...
                const result = upsertChunks(opened.db, chunks);
                const stats = indexStats(opened.db);
                opened.db.close();
            ...
            if (args.action === 'annotate') {
                ...
                const stats = indexStats(opened.db);
                const remaining = untaggedChunks(opened.db, { limit: 9999 }).length;
                opened.db.close();
            ...
            const hits = searchChunks(opened.db, q, { limit, minHitRatio: minHit });
            const stats = indexStats(opened.db);
            opened.db.close();
            if (stats.chunks === 0) {
