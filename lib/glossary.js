...
/** 删除术语；term 不存在则报错（与 worldbook remove 同语义——静默成功会让人
 *  以为删掉了实际没删的东西，拼写错了也毫无察觉）。 */
export function removeTerm(list, term) {
    const before = list ?? [];
    const next = before.filter((x) => x.term !== term);
    if (next.length === before.length) throw new Error(`术语不存在：${term}（novel_glossary list 查看现有术语）`);
    return next;
}
