...
function Btn({ variant = "secondary", size = "md", action, id, disabled = false, title, block = false } = {}, ...label) {
    ...
    h(
      ...
      PHASES.map((p, i) => h("span", {
        ...
        style: {
          ...
          height: "3px",
          borderRadius: radius.pill,
          background: i < idx ? color.accent : i === idx ? color.accent : color.border2,
...
function ProjectDetailView({ state: s }) {
  ...
  const continuityBlock = Card(
    ...
    s.continuityLoading ? h("div", { style: { ...hintStyle, marginTop: space.sm } }, "正在核对账本、伏笔、时间线与文件……") : !cont ? h(
      ...
      cont.issues?.length > 0 ? h("div", null, ...cont.issues.slice(0, 12).map((it, i) => issueRow(it, i, cont.issues.length))) : h(
        "div",
        { style: { ...okStyle, fontSize: font.small } },
        "账本、伏笔、时间线、文件都对得上。"
      )
    ),
    s.continuityError ? h("div", { style: { ...errStyle, marginTop: space.sm } }, s.continuityError) : null
  );
  const proposalsBlock = Card(
    ...
    s.proposalsLoading ? h("div", { style: { ...hintStyle, fontSize: font.caption, marginTop: space.sm } }, "加载中…") : pendingProposals.length === 0 ? h(
      "div",
      { style: { ...hintStyle, fontSize: font.caption, marginTop: space.sm, lineHeight: 1.7 } },
      ...
      ...pendingProposals.map((p) => {
        ...
        return h(
          ...
          h(
            "span",
            { style: { flex: "1 1 auto", minWidth: 0, fontSize: font.small } },
            `第 ${p.chapter} 章`,
  ...
  const editBlock = Card(
    ...
    h(
      ...
      Btn(
        { size: "sm", variant: "danger", action: "delete" },
  ...
  return h(
    ...
    s.loreEntries.length === 0 ? Empty({
      ...
      ...s.loreEntries.map((entry) => h(
        ...
        h(
          "div",
          { style: { display: "flex", gap: space.xs, marginTop: space.sm } },
...
function createForgeController({ sessionId = null, onChange = () => {
  ...
  const loadProposals = async (id) => {
    ...
    try {
      ...
      if (seq === openSeq) state.proposals = [];
    } finally {
      if (seq === openSeq) {
  ...
  const handleAction = async (action, target) => {
    ...
    switch (action) {
      ...
      case "goto-settings":
        ...
        break;
      case "write":
        needsModel("一键写章要拼上下文包并走落盘门禁，只能在会话里做 —— 让 AI 调 novel_write_chapter，或直接说「写第 N 章」。");
        break;
      case "polish":
      ...
      case "draft-batch":
        ...
        break;
      case "diagnose":
        needsModel("结构诊断需要模型判断，请在会话里调 novel_diagnose 或 novel_audit（诊断结果会落到审计里）。");
        break;
      case "import-demo":
      case "import-file":
