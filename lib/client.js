window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 声明的 client 服务：UI 槽注册表 + 右侧 tab 类型注册表。 */
		const inject = ["slots", "sidebarRightTabs"];

		/**
		 * 客户端插件主体：① 先经 sidebarRightTabs.register 创建右侧栏的「锻炉」tab 类型
		 * （id/kind/title；没有这个，内容 seat 再对也不会有 tab 入口）；
		 * ② 再把 tab 面板与标题分别注册进对应 seat，key 用同一 tab id。
		 * 完全对齐 dsh-client-ui-sidebar-files 的 pane.tab 体系（tab-types + 双 seat）。
		 */
		function apply(ctx) {
			...
			ctx.effect(() => ctx.slots.inject("sidebar.right.pane.tab",
				() => ctx.slots.register({ name: "sidebar.right.pane.tab", key: FORGE_TAB_ID }, ForgePanel)),
