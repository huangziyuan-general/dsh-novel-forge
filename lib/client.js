window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function probeRemote(remote, sessionId) {
			...
			// list 的参数形态在类型层是「工作区相对路径」，但调用层是否带 sessionId
			// 需实测确认 —— 逐个试，把每次失败的错误码原样带回，供一次性收敛。
			const attempts = [
				["(sessionId, \"\")", () => wf.list(sessionId, "")],
				["(sessionId, {path})", () => wf.list(sessionId, { path: "" })],
				["(sessionId)", () => wf.list(sessionId)],
				["({sessionId, path})", () => wf.list({ sessionId: sessionId, path: "" })],
			];
			const tried = [];
		...
		function ForgePanel(props) {
			...
			const children = [
				...
				react.createElement("p", { key: "foot", style: footer },
					"在会话中调用 novel_* 工具驱动；本卡显示 Client Remote 的运行时装配情况。"),
			];
		...
		const inject = ["slots", "sidebarRightTabs", "sidebarRight", "remote"];
		...
		exports.__internals = { probeRemote: probeRemote, PLUGIN_VERSION: PLUGIN_VERSION };
