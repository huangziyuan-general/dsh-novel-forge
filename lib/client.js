window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function probeRemote(remote, sessionId) {
			...
			// ── list 的真实契约（对齐 @deepseek-ai/dsh-api-workspace-files）──────────
			//   list(sessionId, path, signal?) ⇒ RemoteResult<{ path, entries, truncated }>
			//   两条硬约束，此前两条都踩了：
			//   ① path 必填且非空；空串是 gateway/bad-request。
			//   ② **list 严格限定在会话工作区内**（read / stat / readBytes 不受此限，
			//      但 list 与 changes 受限）。工作区根 = SessionHeader.cwd，单根且不可变
			//      （见 dsh-sandbox-policy：`process.cwd()` 只是无 cwd 会话的兜底，
			//       正常会话用自己那个 immutable cwd，且不支持额外可写根）。
			//       ⇒ 根路径只能用**工作区相对根 `"."`**。
			//   绝不能拿 `$host.home` 当根：home 是 Host 机器的家目录，通常正是工作区根的
			//   **父目录**，传进去必然 `workspace-file/outside-workspace`。
			//   历史教训（v0.3.4~0.3.6）：本地 mock 里 host.home 恰好等于工作区根，于是
			//   "home 就是根"被测试固化成断言、全绿；接真实 Host 时 100% 失败。测试若跟着
			//   实现走而不是跟着被集成的真实契约走，就会产出这种"全绿但接不上"的假象。
			const attempts = [
				[".", () => wf.list(sessionId, ".")],
				["./", () => wf.list(sessionId, "./")],
			];
			...
			out.rootError = "列工作区根失败：" + tried.join("；")
				+ "（list 只能列会话工作区内的路径；工作区根 = 本会话 cwd）";
			return out;
		}
		...
		async function probeReadBook(wf, sessionId, bookName) {
			const label = bookName || "（工作区根即书）";
			const out = { book: label, files: {}, readForm: null, readError: null, absPath: null, summary: null };
			if (!wf || typeof wf.read !== "function") {
				out.readError = "workspaceFiles.read 不可用";
				return out;
			}
			// bookName 为空 ⇒ 工作区根本身就是一本锻炉书：路径**不能加前导斜杠**，
			// 否则会变成绝对路径 "/novel.json" 而绕过工作区根。
			const pre = bookName ? bookName + "/" : "";
			// 必需/可选：novel.json 定义一本书，读不出就不算书；其余（facts/伏笔/style）都是
			// 数据累计后才产生的文件——一本新书没建账、没跑过风格基线是**正常状态**，
			// 缺失应静默当成"零/未构建"，绝不能当成读盘错误在面板上刷红。
			const specs = [
				["novel", pre + "novel.json", true],
				["facts", pre + "账本/facts.json", false],
				["foreshadows", pre + "账本/伏笔.json", false],
				["style", pre + ".novel/style-baseline.json", false],
			];
			// read 的真实契约：read(sessionId, path, range, signal?) —— **range 是必填对象**。
			// 缺参或传 undefined 会以装配错误（arity）直接 reject，这正是 v0.3.4~0.3.6
			// 读盘全败的原因（此前形态写成 (sessionId, path) 与 (sessionId, path, undefined)）。
			// 空对象 {} 即取服务端默认页（offset=1、limit=maxLines=5000 行）。
			// 另注：read 不受工作区边界限制（只有 list/changes 受限），所以路径可以是绝对路径。
			const forms = [
				(p) => wf.read(sessionId, p, {}, undefined),
				(p) => wf.read(sessionId, p, {}),
			];
			const failures = [];
			for (const [kind, rel, required] of specs) {
				let text = null, lastErr = null;
				for (let i = 0; i < forms.length; i++) {
					try {
						const res = await forms[i](rel);
						if (res && res.ok === false) { lastErr = describeFailure(res); continue; }
						const val = res && res.value !== undefined ? res.value : res;
						// WorkspaceFileText = { offset, text, lines, eof, absolutePath, version, bytes? }
						const body = (val && typeof val === "object" && typeof val.text === "string") ? val.text : val;
						if (typeof body === "string") {
							text = body;
							if (out.readForm === null) out.readForm = kind + "@" + i;
							if (out.absPath === null && val && typeof val.absolutePath === "string") out.absPath = val.absolutePath;
							break;
						}
					} catch (e) { lastErr = (e && e.message) || "抛错"; }
				}
				if (text === null) {
					// 必需的收进 readError；可选的缺失是合法状态，静默跳过（摘要里自动视为空）
					if (required) failures.push(kind + ": " + (lastErr || "无返回文本"));
				} else {
					out.files[kind] = text;
				}
			}
			if (failures.length) out.readError = failures.join("；");
			// novel.json 读不到 → 不造幽灵书目，只留 readError（可选文件全读到也只算陪衬）
			if (out.files.novel != null) {
		...
		/** 一个目录是不是锻炉书：里面有没有 novel.json。
		 *  探测失败（无 list / 报错 / 抛错）一律当"不是书"——候选筛选宁缺勿滥，绝不抛。 */
		async function dirHasNovel(wf, sessionId, dir) {
			if (!wf || typeof wf.list !== "function") return false;
			try {
				const res = await wf.list(sessionId, dir);
				if (!res || !res.ok) return false;
				const entries = (res.value && res.value.entries) || [];
				return entries.some((e) => e && e.name === "novel.json" && e.type === "file");
			} catch (e) { return false; }
		}

		/** 枚举工作区里的锻炉书，逐个读盘出控制台摘要。
		 *  两态判定（都按 lib/store.js pathsFor 的真实布局）：
		 *   ① 工作区根下直接有 novel.json ⇒ 工作区根本就**是一本**书 → bookName 传空串；
		 *   ② 否则扫描工作区根的一级子目录，**只保留含 novel.json 的**当书。
		 *
		 *  为什么"先探再读"：一级目录里往往大量是无关项目目录（lib/ test/、甚至与工作区同名的
		 *  仓库目录），对每个目录盲读 4 个机器文件会刷出满屏 workspace-file/not-found 噪声，
		 *  既浪费请求又把真实错误淹没。用 1 次轻量 list 换掉 4 次 read。
		 *
		 *  上限 MAX_SCAN / MAX_BOOKS 防止工作区被塞进巨型目录时打爆面板。 */
		const MAX_SCAN = 30;
		const MAX_BOOKS = 12;
		async function loadBookConsole(remote, sessionId, probe) {
			...
			const selfIsBook = entries.some((x) => x && x.name === "novel.json" && x.type === "file");
			if (selfIsBook) return [await probeReadBook(wf, sessionId, "")];
			const dirs = entries.filter((x) => x && x.type === "directory" && x.name).map((x) => x.name);
			const books = [];
			for (const d of dirs.slice(0, MAX_SCAN)) {
				if (await dirHasNovel(wf, sessionId, d)) books.push(d);
				if (books.length >= MAX_BOOKS) break;
			}
			const out = [];
			for (const d of books) out.push(await probeReadBook(wf, sessionId, d));
			return out;
		}
		...
		function renderBooks(books) {
			...
			if (!books.length) {
				return react.createElement("div", { key: "books", style: card },
					...
					react.createElement("div", { style: row },
						...
						react.createElement("span", { style: v }, "未发现锻炉书（需 <书名>/novel.json，或工作区根本身是一本书）")),
					react.createElement("div", { style: row },
						react.createElement("span", { style: k }, "修复"),
						react.createElement("span", { style: mono },
							"list 只能列会话工作区内的路径 → 让 dsh 从书库目录启动：DSH_PROJECT=<书库目录> ~/restart-dsh.sh")));
			}
			const rows = books.map((b) => {
				...
				const err = b.readError
					...
					: null;
				// 读到盘时把 Host 返回的绝对路径亮出来——它是"确实命中真实文件"的凭证。
				const abs = b.absPath
					? react.createElement("div", { style: row },
						react.createElement("span", { style: k }, "盘"),
						react.createElement("span", { style: mono }, b.absPath))
					: null;
		...
		function renderProbe(probe, sessionId) {
			...
			if (probe.rootEntries) {
				...
				lines.push(react.createElement("div", { key: "r", style: row },
					react.createElement("span", { style: k }, "workspace"),
					react.createElement("span", { style: mono },
						"根 " + (listing.path || "?") + " ← " + probe.rootEntries.via
						+ " · " + entries.length + " 项" + (listing.truncated ? "（已截断）" : "")
		...
		exports.__internals = {
			...
			loadBookConsole: loadBookConsole,
			dirHasNovel: dirHasNovel,
